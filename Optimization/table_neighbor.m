function new_table_ar = table_neighbor(old_table_ar,split_merge)
...
% total number of seats remaings the same.
% 
% -------------------------------------------------------------------------
% PARAMETERS
% 
...
if r1<split_merge   % Split tables.
    probs = [0, repmat(0.25,1,4)];    % Probability vector to choose tables to split (Table of size 1 cannot be splitted).
    ...
    options=table_options(k);
    r3=randi(size(options,1));
    for l=1:size(options,2)
        new_table_ar(options(r3,l))=new_table_ar(options(r3,l))+1;
    end
else    % Merge tables.
    probs = [0, repmat(0.25,1,4)];    % Probability vector to choose the size of the merged new table.
